use gtk::{
    Adjustment, ButtonExt, GridExt, LabelExt, OrientableExt, Orientation, SpinButtonExt, SpinButtonSignals
};
use relm::{Widget};
...
impl Widget for PatternContainer {
	fn model() -> PatternContainerModel {
		PatternContainerModel {
			a: 2.0,
		}
	}

	fn update(&mut self, event: PatternContainerMsg) {
		match event {
			_ => (),
		}
	}
